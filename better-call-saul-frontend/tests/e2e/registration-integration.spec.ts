...
test.describe('Registration and Login Integration Tests', () => {
  test('should complete real registration and login flow with valid registration code', async ({ page }) => {
    ...
    await page.fill('input[name="lastName"], input[placeholder*="Last"], input[id*="last"]', testUser.lastName);
    await page.fill('input[name="email"], input[placeholder*="Email"], input[type="email"]', testUser.email);
    await page.fill('input[name="registrationCode"], input[placeholder*="Registration"], input[placeholder*="Code"]', testUser.registrationCode);
    await page.fill('input[name="password"], input[type="password"]:not([name*="confirm"])', testUser.password);
    await page.fill('input[name="confirmPassword"], input[placeholder*="Confirm"]', testUser.password);
    
    // Optional fields
    const barNumberField = page.locator('input[name="barNumber"], input[placeholder*="Bar"]').first();
    if (await barNumberField.isVisible()) {
      await barNumberField.fill(testUser.barNumber);
    }
    
    const lawFirmField = page.locator('input[name="lawFirm"], input[placeholder*="Law Firm"]').first();
    ...
    // Submit registration
    const submitButton = page.locator('button[type="submit"]').or(page.locator('text=Create Account')).or(page.locator('text=Register')).first();
  ...
  test('should show error for invalid registration code', async ({ page }) => {
    ...
    await page.fill('input[name="registrationCode"], input[placeholder*="Registration"]', invalidUser.registrationCode);
    await page.fill('input[name="password"], input[type="password"]:not([name*="confirm"])', invalidUser.password);
    await page.fill('input[name="confirmPassword"], input[placeholder*="Confirm"]', invalidUser.password);
